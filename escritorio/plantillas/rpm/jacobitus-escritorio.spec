...
Autoprov: 0
Autoreq: 0
%if "xalsa-lib, bzip2-libs, freetype, glib2, glibc, graphite2, harfbuzz, libX11, libXau, libXext, libXi, libXrender, libXtst, libbrotli, libgcc, libpng, libstdc++, libxcb, pcre2, xdg-utils, zlib" != "x" || "x@REQUIRES@" != "x"
Requires: alsa-lib, bzip2-libs, freetype, glib2, glibc, graphite2, harfbuzz, libX11, libXau, libXext, libXi, libXrender, libXtst, libbrotli, libgcc, libpng, libstdc++, libxcb, pcre2, xdg-utils, zlib @REQUIRES@
%endif
...
cp -r %{_sourcedir}@SOURCE_DIR@/* %{buildroot}@INSTALL_DIR@
%if "x@LICENSE_FILE@" != "x"
  %define license_install_file %{_defaultlicensedir}/%{name}-%{version}/%{basename:@LICENSE_FILE@}
  install -d -m 755 "%{buildroot}%{dirname:%{license_install_file}}"
  install -m 644 "@LICENSE_FILE@" "%{buildroot}%{license_install_file}"
%endif
...
xdg-mime uninstall @MIME_FILE@
uninstall_default_mime_handler @UNINSTALL_MIME@
%clean